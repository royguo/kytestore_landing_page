...
- 日常改进记录统一放在 `docs/worklog/` 目录。
- 工作日志使用 Markdown 编写，文件名采用 `YYYY-MM-DD.md` 格式，例如 `docs/worklog/2026-06-24.md`。
- 工作日志应记录当天完成的用户可见改进、内容更新、验证结果和必要的后续事项。
- 如果一次改动涉及源文件和 `public/` 部署副本，工作日志中应明确两者是否都已同步。
- 不要把临时想法、未验证判断或敏感凭据写入工作日志。
